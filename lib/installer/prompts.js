...
export async function runInstallPrompts(detectedEngines) {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      choices: [
        { name: `Pipeline Core ${chalk.gray('(extract, planner, copywriter, builder, animator, validator)')}`, value: 'core', checked: true, disabled: 'sempre instalado' },
    ...
    },
    {
      ...
      type: 'input',
      name: 'first_source',
      message: promptTitle(7, 'Caminho de uma fonte de conteúdo para vincular agora (Enter para pular):'),
      default: '',
    },
    {
      ...P,
      type: 'list',
      name: 'git_strategy',
      message: promptTitle(8, 'Como tratar os decks no git?', 'list'),
      loop: false,
      choices: [
