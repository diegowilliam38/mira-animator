...
export default async function install() {
  ...
  // 6. Primeira fonte (opcional)
  const firstSource = (answers.first_source || '').trim();
  if (firstSource) {
    const srcPath = resolve(firstSource);
    if (existsSync(srcPath)) {
      config.sources = config.sources ?? [];
      config.sources.push({
        name: basename(srcPath).replace(/^\./, '').replace(/\.[^.]+$/, ''),
        path: srcPath,
        type: 'projeto',
        linkedAt: new Date().toISOString(),
      });
      console.log(chalk.hex('#FF904D')('  ✔') + `  fonte vinculada: ${srcPath}`);
    } else {
      console.log(chalk.yellow(`  ⚠  fonte não encontrada, pulei: ${srcPath}`));
    }
  }
  saveConfig(config);
