...
Genera la escena del **"prompt tecleado en zoom"**: una sola línea de fuente monoespaciada de terminal, gigante, sobre fondo oscuro, tecleada carácter a carácter con un cursor de barra parpadeante estilo Windows, simulando a alguien escribiendo un comando para un agent harness (Claude Code, Codex). Cuando el punto de tecleo alcanza 100px antes del borde derecho, todo el texto se desliza a la izquierda manteniendo el cursor anclado (ventana deslizante). Pasas el texto como argumento y puedes colorear tramos con la etiqueta `color=#HEX` (la etiqueta nunca aparece; el resto queda blanco). Al terminar, pausa con el cursor parpadeando y reinicia en bucle. JS/CSS puro, cero dependencias, funciona desde `file://`.
